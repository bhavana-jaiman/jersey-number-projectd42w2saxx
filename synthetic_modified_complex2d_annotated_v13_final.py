...
def process_source(image_path,label_path,simple_image_path,number,out_path,image_size,args):
    ...
    target_h=max(8,int(round(H*random.uniform(
        args.synthetic_height_min_ratio,
        args.synthetic_height_max_ratio
    ))))
...
def main():
    ap=argparse.ArgumentParser(description="Complex2D v12: independent synthetic-number size, exact-label inpainting region, placement from real label, final-mask digit annotations")
    ...
    ap.add_argument("--synthetic-height-min-ratio","--min-height-ratio",dest="synthetic_height_min_ratio",type=float,default=.10,help="Minimum synthetic-number height as a fraction of the REAL input image/crop height; default 10%%")
    ap.add_argument("--synthetic-height-max-ratio","--max-height-ratio",dest="synthetic_height_max_ratio",type=float,default=.18,help="Maximum synthetic-number height as a fraction of the REAL input image/crop height; default 18%%")
