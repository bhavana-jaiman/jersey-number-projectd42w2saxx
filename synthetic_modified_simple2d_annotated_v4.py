...
Annotation format (normalized YOLO-style, one line per digit):
    <digit> <x_center> <y_center> <width> <height>
For a double-digit number such as 28, the label file contains two lines: one for 2 and one for 8.
...
def render_number_with_mask(number,image_size=100,font_path=None,bg_color=(20,30,60),text_color=(255,255,255),scale_range=(.48,.70),gap_ratio=(0,.02),double_scale_range=(.20,.34)):
    """
    Render the complete jersey number as one coherent visual object, while
    also keeping a separate mask for each digit so annotations can be emitted
    separately for double-digit numbers.

    Example for 28:
        visual rendering: "28" as one number
        returned digit boxes: [box(2), box(8)]
    """
    n=int(number)
    ...
    if len(s)==1:
        ...
        dm=Image.new("L",(image_size,image_size),0)
        ImageDraw.Draw(dm).text(pos,s,font=font,fill=255)
        digit_masks.append(np.asarray(dm).copy())
    else:
        # v3: double digits are rendered with a compact font scale so their
        # normalized center distance matches the real dataset and stays <0.25.
        font=load_font(font_path,max(8,int(image_size*random.uniform(*double_scale_range))))
        ...
        # Render both digits together as one coherent number.
        ...
        # Separate digit masks are used only for annotation generation.
        dm1=Image.new("L",(image_size,image_size),0)
        dm2=Image.new("L",(image_size,image_size),0)
        ImageDraw.Draw(dm1).text(p1,s[0],font=font,fill=255)
        ImageDraw.Draw(dm2).text(p2,s[1],font=font,fill=255)
        digit_masks.extend([np.asarray(dm1).copy(),np.asarray(dm2).copy()])

    digit_bboxes=[]
    for dm in digit_masks:
        box=_bbox(dm) if '_bbox' in globals() else None
        # _bbox is defined immediately after this function; use the same
        # calculation locally so this function is also self-contained.
        if box is None:
            ys,xs=np.where(dm>0)
            box=None if len(xs)==0 else (int(xs.min()),int(ys.min()),int(xs.max()),int(ys.max()))
        if box is not None:
            digit_bboxes.append(box)

    return np.asarray(im).copy(),np.asarray(mask).copy(),digit_bboxes
...
def write_annotation_txt(path,digit_bboxes,image_size=None):
    """Write one annotation line per digit.

    For a double digit number such as 28, the file contains two lines:
        2 <bbox>
        8 <bbox>
    """
    w,h=image_size
    lines=[]
    for digit,bbox in digit_bboxes:
        x1,y1,x2,y2=bbox
        xc=((x1+x2)/2)/w; yc=((y1+y2)/2)/h
        bw=(x2-x1+1)/w; bh=(y2-y1+1)/h
        lines.append(f"{digit} {xc:.6f} {yc:.6f} {bw:.6f} {bh:.6f}")
    path.write_text("\n".join(lines)+("\n" if lines else ""))

def generate_simple2d_class(number,out_dir,count,image_size,font_path,rows,gap_min=0.0,gap_max=0.02):
    out_dir.mkdir(parents=True,exist_ok=True)
    colors=list(BG_COLORS.values()); per=max(1,math.ceil(count/len(colors))); samples=[]
    for bg in colors:
        for _ in range(per):
            im,mask,digit_bboxes=render_number_with_mask(number,image_size,font_path,bg,choose_text_color(bg),gap_ratio=(gap_min,gap_max))
            samples.append((im,mask,digit_bboxes))
    random.shuffle(samples)
    digits=str(int(number))
    for i,(im,mask,digit_bboxes) in enumerate(samples[:count]):
        if not digit_bboxes: continue
        ...
        # IMPORTANT: annotations are digit-level, not whole-number-level.
        digit_bbox_pairs=list(zip([int(ch) for ch in digits],digit_bboxes))
        write_annotation_txt(txt_path,digit_bbox_pairs,(image_size,image_size))

        for digit,bbox in digit_bbox_pairs:
            x1,y1,x2,y2=bbox
            rows.append([str(img_path.resolve()),number,digit,x1,y1,x2,y2])

def write_csv(rows,path):
    with path.open("w",newline="") as f:
        w=csv.writer(f); w.writerow(["image_path","jersey_number","digit","bbox_x1","bbox_y1","bbox_x2","bbox_y2"]); w.writerows(rows)
...
def main():
    ap=argparse.ArgumentParser(description="Simple2D v3 jersey-number generator with close double-digit spacing and per-digit annotations")
    ...
    ap.add_argument("--gap-max",type=float,default=0.02,help="Maximum double-digit gap as fraction of digit width; v3 also uses compact double-digit rendering to keep center distance below 0.25")
    ...
    for n in nums:
        if counts[n]:
            ...
            generate_simple2d_class(n,a.output/str(n),counts[n],a.image_size,a.font,rows,a.gap_min,a.gap_max)
