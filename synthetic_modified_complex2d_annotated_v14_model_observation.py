...
Version 14: model-observation-driven small-scale and rotation augmentation.
...
def sample_rotation(rotation_min=-45.0, rotation_max=45.0):
    """Sample rotation with extra emphasis on the difficult +/-30 to +/-45 range."""
    u=random.random()
    if u < 0.20:
        angle=0.0
    elif u < 0.35:
        ...
        angle=random.choice([-1.0,1.0])*random.uniform(10.0,30.0)
    else:
        angle=random.choice([-1.0,1.0])*random.uniform(30.0,45.0)
    return max(rotation_min,min(rotation_max,angle))


def sample_synthetic_height_ratio(args):
    """Oversample the scale ranges where the model currently performs worst."""
    r=random.random()
    p1=args.small_scale_bin1_prob
    p2=p1+args.small_scale_bin2_prob
    p3=p2+args.medium_scale_prob
    p4=p3+args.normal_scale_prob
    if r < p1:
        return random.uniform(args.small_scale_bin1_min,args.small_scale_bin1_max)
    if r < p2:
        return random.uniform(args.small_scale_bin2_min,args.small_scale_bin2_max)
    if r < p3:
        return random.uniform(args.medium_scale_min,args.medium_scale_max)
    if r < p4:
        return random.uniform(args.normal_scale_min,args.normal_scale_max)
    return random.uniform(args.large_scale_min,args.large_scale_max)


def warp_number_object(patch,mask,rotation_min=-45,rotation_max=45,perspective_min=.025,perspective_max=.075,grid_prob=.40,grid_strength_min=.015,grid_strength_max=.035,noise_prob=.30,noise_min=.5,noise_max=2.0,small_scale=False):
    ph,pw=patch.shape[:2]
    ...
    angle=sample_rotation(rotation_min,rotation_max)
    direction=random.choices(["straight","left","right"],weights=[.30,.35,.35])[0]
    M=cv2.getRotationMatrix2D((nw/2,nh/2),angle,1.0); c=abs(M[0,0]); ss=abs(M[0,1]); rw=max(8,int(nh*ss+nw*c)); rh=max(8,int(nh*c+nw*ss)); M[0,2]+=rw/2-nw/2; M[1,2]+=rh/2-nh/2
    ...
    # Small digits are already difficult. Keep grid/noise milder in the 5-14 px
    # target bins so augmentation does not destroy the strokes we need to learn.
    if small_scale:
        effective_grid_prob=min(grid_prob,.18); effective_noise_prob=min(noise_prob,.12)
        effective_grid_min=min(grid_strength_min,.008); effective_grid_max=min(grid_strength_max,.018)
        effective_noise_min=min(noise_min,.25); effective_noise_max=min(noise_max,.90)
    else:
        effective_grid_prob=grid_prob; effective_noise_prob=noise_prob
        effective_grid_min=grid_strength_min; effective_grid_max=grid_strength_max
        effective_noise_min=noise_min; effective_noise_max=noise_max
    if random.random()<effective_grid_prob:
        patch=grid_distortion(patch,random.uniform(effective_grid_min,effective_grid_max))
    if random.random()<effective_noise_prob:
        patch=gaussian_noise(patch,(effective_noise_min,effective_noise_max))
...
def expanded_group_bbox(group,W,H,pad_ratio=0.0):
    ...
    pad=max(0,int(round(max(pw,ph)*pad_ratio)))
    return max(0,x1-pad),max(0,y1-pad),min(W-1,x2+pad),min(H-1,y2+pad)

def select_number_group(boxes,W,H,distance_ratio=.75,y_center_ratio=.40,height_ratio=.55,pad_ratio=0.0):
    ...
    if doubles:
        ...
        return expanded_group_bbox(g,W,H,pad_ratio),g
...
def remove_number_region(image, old_bbox, method="inpaint", radius=5, blur_kernel=21):
    """Remove ONLY the exact region covered by the selected existing label(s).

    The inpainting radius controls how far OpenCV searches for source pixels;
    it does NOT enlarge the annotation mask. No dilation is applied.
    """
    ...
    # IMPORTANT: do not dilate the mask. The inpainted region must remain
    # exactly the existing label/group region selected above.
    return cv2.inpaint(image,mask,float(max(1,int(radius))),cv2.INPAINT_TELEA)
...
def process_source(image_path,label_path,simple_image_path,number,out_path,image_size,args):
    ...
    old_bbox,number_group=select_number_group(
        ...
        args.remove_pad_ratio
    )
    if old_bbox is None:
        return False,"no_number_group"
    # Remove ONLY the selected existing-label region. The real label is used
    # as a placement/removal reference, never as the synthetic annotation.
    ...
    target_ratio=sample_synthetic_height_ratio(args)
    small_scale=target_ratio < args.small_scale_cutoff
    patch,mask=warp_number_object(patch,mask,args.rotation_min,args.rotation_max,args.perspective_min,args.perspective_max,args.grid_prob,args.grid_strength_min,args.grid_strength_max,args.noise_prob,args.noise_min,args.noise_max,small_scale=small_scale)
    ...
    # V11 DESIGN CHANGE:
    # The OLD jersey-number bbox is used ONLY to determine the placement
    # location (center) and the region to inpaint. It is NOT used to size
    # or fit the synthetic number.
    #
    # The synthetic number gets an independent, controlled height based on
    # the REAL input image/crop height. This prevents very large source
    # annotations from producing oversized numbers and very small source
    # annotations from producing tiny numbers.
    target_h=max(8,int(round(H*target_ratio)))
    sc=target_h/max(patch.shape[0],1)
    nw=max(8,int(round(patch.shape[1]*sc)))
    nh=max(8,int(round(patch.shape[0]*sc)))
    patch=cv2.resize(patch,(nw,nh),cv2.INTER_CUBIC)
    mask=cv2.resize(mask,(nw,nh),cv2.INTER_LINEAR)
    ...
    # IMPORTANT: digit_bboxes_from_mask() returns coordinates relative to the
    # transformed number patch. After paste_center(), convert them to
    # coordinates in the FULL output image by adding the paste offset (px, py).
    # Without this step, labels incorrectly appear near (0, 0).
    digit_bboxes=[
        (
            int(round(px + bx1)),
            int(round(py + by1)),
...
def main():
    ap=argparse.ArgumentParser(description="Complex2D v14: model-observation-driven scale + rotation augmentation, exact-label inpainting, final-mask digit annotations")
    ...
    ap.add_argument("--remove-pad-ratio",type=float,default=0.0,help="Extra margin around old-number labels before removal; default 0 = exact existing-label region")
    ...
    # V14: deliberately oversample the scale bins where evaluation is weakest.
    ap.add_argument("--synthetic-height-min-ratio","--min-height-ratio",dest="synthetic_height_min_ratio",type=float,default=.05,help="Overall minimum synthetic-number height ratio safety bound")
    ap.add_argument("--synthetic-height-max-ratio","--max-height-ratio",dest="synthetic_height_max_ratio",type=float,default=.27,help="Overall maximum synthetic-number height ratio safety bound")
    ap.add_argument("--small-scale-bin1-min",type=float,default=.05,help="Lower bound of ~5-9 px-equivalent scale for 100 px crops")
    ap.add_argument("--small-scale-bin1-max",type=float,default=.09,help="Upper bound of ~5-9 px-equivalent scale for 100 px crops")
    ap.add_argument("--small-scale-bin2-min",type=float,default=.09,help="Lower bound of ~9-14 px-equivalent scale for 100 px crops")
    ap.add_argument("--small-scale-bin2-max",type=float,default=.14,help="Upper bound of ~9-14 px-equivalent scale for 100 px crops")
    ap.add_argument("--medium-scale-min",type=float,default=.14,help="Lower bound of ~14-18 px-equivalent scale")
    ap.add_argument("--medium-scale-max",type=float,default=.18,help="Upper bound of ~14-18 px-equivalent scale")
    ap.add_argument("--normal-scale-min",type=float,default=.18,help="Lower bound of ~18-23 px-equivalent scale")
    ap.add_argument("--normal-scale-max",type=float,default=.23,help="Upper bound of ~18-23 px-equivalent scale")
    ap.add_argument("--large-scale-min",type=float,default=.23,help="Lower bound of 23-27 px-equivalent tail")
    ap.add_argument("--large-scale-max",type=float,default=.27,help="Upper bound of 23-27 px-equivalent tail")
    ap.add_argument("--small-scale-bin1-prob",type=float,default=.30,help="Probability of 5-9 px-equivalent target size")
    ap.add_argument("--small-scale-bin2-prob",type=float,default=.30,help="Probability of 9-14 px-equivalent target size")
    ap.add_argument("--medium-scale-prob",type=float,default=.20,help="Probability of 14-18 px-equivalent target size")
    ap.add_argument("--normal-scale-prob",type=float,default=.15,help="Probability of 18-23 px-equivalent target size")
    ap.add_argument("--small-scale-cutoff",type=float,default=.14,help="Below this ratio, use gentler grid/noise")
    ...
    # Backward-compatible options from V10/V11. In V13 the synthetic number is
    # NOT fitted to the old label bbox. These options are accepted so existing
    # commands do not fail; --size-mode is retained as a compatibility flag.
    ap.add_argument("--size-mode",choices=["original","enlarge"],default="enlarge",help="Compatibility option; V13 always uses independent synthetic sizing")
    ap.add_argument("--scale-factor",type=float,default=1.10,help="Compatibility option; independent-size mode does not fit to old bbox")
    ap.add_argument("--keep-aspect",action=argparse.BooleanOptionalAction,default=True)
    ap.add_argument("--rotation-min",type=float,default=-45.0); ap.add_argument("--rotation-max",type=float,default=45.0)
    ...
    if not (0 < a.synthetic_height_min_ratio <= a.synthetic_height_max_ratio):
        raise ValueError("synthetic height ratios must satisfy 0 < min <= max")
    probs=(a.small_scale_bin1_prob,a.small_scale_bin2_prob,a.medium_scale_prob,a.normal_scale_prob)
    if any(p < 0 for p in probs) or sum(probs) > 1.000001:
        raise ValueError("scale-bin probabilities must be non-negative and sum to <= 1")
    for lo,hi in ((a.small_scale_bin1_min,a.small_scale_bin1_max),(a.small_scale_bin2_min,a.small_scale_bin2_max),(a.medium_scale_min,a.medium_scale_max),(a.normal_scale_min,a.normal_scale_max),(a.large_scale_min,a.large_scale_max)):
        if lo <= 0 or lo > hi:
            raise ValueError("each scale-bin must satisfy 0 < min <= max")
    if a.synthetic_height_min_ratio > a.small_scale_bin1_min or a.synthetic_height_max_ratio < a.large_scale_max:
        raise ValueError("overall synthetic height bounds must contain all scale bins")
