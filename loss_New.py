...
# ---------------HCL_Changes---------------
# power=1.0 gives the original weights (total / (num_classes * count)).
# power=0.5 (square root) softens them, so a rare class is boosted less
# aggressively. Weights are normalised so the average weight over the data
# is 1, which keeps the loss on the same scale. Counts of 0 are treated as 1.
...
def calculate_weights(counts, power: float = 1.0):
    counts = np.maximum(np.array(counts, dtype=np.float32), 1.0)
    freq = counts / counts.sum()
    class_weights = (1.0 / freq) ** power
    class_weights = class_weights / (freq * class_weights).sum()
    return torch.tensor(class_weights, dtype=torch.float32)
# -------------------------------------------
...
class make_loss_fn(nn.Module):
    ...
    # New options:
    #   weight_power    : 0.5 = softened (square-root) state weights, 1.0 = original
    #   label_smoothing : for digit1/digit2 (0.0 = off); helps with noisy/cut labels
    def __init__(self, state_class_counts=None, lambda_state: float = 1.0,
                 weight_power: float = 0.5, label_smoothing: float = 0.1) -> None:
        # -------------------------------------------
        super(make_loss_fn, self).__init__()

        # ---------------HCL_Changes---------------
        # Removed: weights_d2 was never used, and the hard-coded "cuda"
        # crashed on machines without a GPU.
        ...
        if state_class_counts is not None:
            state_weights = calculate_weights(state_class_counts, power=weight_power)
        else:
            state_weights = None
        ...
        self.criterion1 = nn.CrossEntropyLoss(ignore_index=IGNORE_INDEX,
        ...
        # the three parts of the last computed loss, for logging in train_New.py
        self.last_losses = {}
        # -------------------------------------------
    ...
    def forward(self, digit1_logits: torch.Tensor, digit2_logits: torch.Tensor,
        ...
        self.last_losses = {"d1": digit1_loss.detach(), "d2": digit2_loss.detach(),
