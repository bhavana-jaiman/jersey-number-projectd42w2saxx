#!/usr/bin/env python3
"""Simple2D + orientation-aware Complex2D jersey-number generator."""
...
def generate_simple2d_class(number,out_dir,count=4000,image_size=100,font_path=None):
    ...
    for bg in colors:
        for _ in range(per):
            ...
            images.append(apply_random_paper_augmentation(np.asarray(im)))
...
def warp_number_object(patch,mask,target_size=100):
    ph,pw=patch.shape[:2]; sc=random.uniform(.92,1.08)
    nw,nh=max(8,int(pw*sc)),max(8,int(ph*sc))
    patch=cv2.resize(patch,(nw,nh),cv2.INTER_CUBIC); mask=cv2.resize(mask,(nw,nh),cv2.INTER_NEAREST)
    level=random.choices(["mild","medium","strong"],weights=[.35,.45,.20])[0]
    if level=="mild": angle=random.uniform(-5,5); ps=random.uniform(.01,.035)
    elif level=="medium": angle=random.uniform(-10,10); ps=random.uniform(.03,.07)
    else: angle=random.uniform(-15,15); ps=random.uniform(.06,.10)
    direction=random.choices(["straight","left","right"],weights=[.30,.35,.35])[0]
    M=cv2.getRotationMatrix2D((nw/2,nh/2),angle,1)
    c,s=abs(M[0,0]),abs(M[0,1]); rw=max(8,int(nh*s+nw*c)); rh=max(8,int(nh*c+nw*s))
    M[0,2]+=rw/2-nw/2; M[1,2]+=rh/2-nh/2
    patch=cv2.warpAffine(patch,M,(rw,rh),cv2.INTER_CUBIC,borderMode=cv2.BORDER_REFLECT_101)
    mask=cv2.warpAffine(mask,M,(rw,rh),cv2.INTER_LINEAR,borderMode=cv2.BORDER_CONSTANT)
    patch,mask=directional_bend(patch,mask,ps,direction)
    return surface_warp(patch,mask,random.uniform(.008,.030))

def resize_number_for_jersey(patch,mask,image_size=100):
    ph,pw=patch.shape[:2]; target_h=int(image_size*random.uniform(.38,.52))
...
def blend_number(bg,patch,mask,x,y):
    ...
    p=patch[:y2-y,:x2-x]; m=mask[:y2-y,:x2-x].astype(np.float32)/255
    m=cv2.GaussianBlur(m,(3,3),0)*random.uniform(.88,.98)
    fg=np.clip(p.astype(np.float32)*random.uniform(.82,1.08),0,255)
    if random.random()<.30: fg=cv2.GaussianBlur(fg,(3,3),0)
    base=bg[y:y2,x:x2].astype(np.float32)
    bg[y:y2,x:x2]=np.clip(fg*m[...,None]+base*(1-m[...,None]),0,255).astype(np.uint8)
    return bg

def generate_complex2d_class(number,simple_root,backgrounds,out_dir,count=4000,image_size=100):
    ...
    while made<count and attempts<max(100,count*5):
        ...
        if random.random()<.25: bg=cv2.GaussianBlur(bg,(3,3),0)
        if random.random()<.20: bg=gaussian_noise(bg,(.8,4))
        cv2.imwrite(str(out_dir/f"{number}_{made:05d}.jpg"),cv2.cvtColor(bg,cv2.COLOR_RGB2BGR),[cv2.IMWRITE_JPEG_QUALITY,95])
        made+=1
...
def generate_complex2d(root,simple_root,bg_dir,count_per_class=4000,image_size=100,numbers=range(100),
    ...
    for n in numbers:
        if counts[n]: print(f"[Complex2D] {n}: {counts[n]}"); generate_complex2d_class(n,simple_root,backgrounds,root/str(n),counts[n],image_size)
