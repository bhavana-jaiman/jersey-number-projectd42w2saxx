...
where the coordinates are normalized to the output image. A CSV summary with
pixel bboxes is also written.
"""
...
def read_yolo_boxes(txt_path,class_id=0):
    boxes=[]
...
def combine_boxes(boxes,W,H,pad_ratio=.03):
    px=[yolo_to_pixel(b,W,H) for b in boxes]
    x1=min(b[0] for b in px); y1=min(b[1] for b in px); x2=max(b[2] for b in px); y2=max(b[3] for b in px)
    pw=x2-x1+1; ph=y2-y1+1; pad=max(1,int(round(max(pw,ph)*pad_ratio)))
    return max(0,x1-pad),max(0,y1-pad),min(W-1,x2+pad),min(H-1,y2+pad)

def inpaint_number(image,old_bbox,radius=3):
    x1,y1,x2,y2=old_bbox; m=np.zeros(image.shape[:2],np.uint8); m[y1:y2+1,x1:x2+1]=255
    # Slightly expand so old digit edges are removed too.
    k=max(1,int(radius)); kernel=cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(2*k+1,2*k+1)); m=cv2.dilate(m,kernel)
    return cv2.inpaint(image,m,float(radius),cv2.INPAINT_TELEA)
...
def paste_center(bg,patch,mask,cx,cy):
    h,w=bg.shape[:2]; ph,pw=patch.shape[:2]; x=int(round(cx-pw/2)); y=int(round(cy-ph/2)); x=max(0,min(x,w-pw)); y=max(0,min(y,h-ph));
    roi=bg[y:y+ph,x:x+pw]; a=(mask.astype(np.float32)/255.0)[...,None]; bg[y:y+ph,x:x+pw]=np.clip(patch.astype(np.float32)*a+roi.astype(np.float32)*(1-a),0,255).astype(np.uint8)
    return bg,x,y
...
def process_source(image_path,label_path,simple_image_path,number,out_path,image_size,args):
    ...
    old_bbox=combine_boxes(boxes,W,H,args.remove_pad_ratio)
    clean=inpaint_number(img,old_bbox,args.inpaint_radius)
    ...
    if args.size_mode == "original":
        # Match the size of the old annotated jersey number.
    ...
    clean,px,py=paste_center(clean,patch,mask,cx,cy)
    # Resize whole crop to requested output size and scale the exact resulting bbox.
    sx=image_size/W; sy=image_size/H; out=cv2.resize(clean,(image_size,image_size),cv2.INTER_AREA)
    # Recover bbox from transformed mask placed on original coordinates.
    mb=bbox(mask)
    if mb is None:return False,"empty_mask"
    mx1,my1,mx2,my2=mb; fx1=px+mx1; fy1=py+my1; fx2=px+mx2; fy2=py+my2
    x1=int(round(fx1*sx)); y1=int(round(fy1*sy)); x2=int(round(fx2*sx)); y2=int(round(fy2*sy)); x1=max(0,min(image_size-1,x1)); y1=max(0,min(image_size-1,y1)); x2=max(x1,min(image_size-1,x2)); y2=max(y1,min(image_size-1,y2))
    cv2.imwrite(str(out_path),out,[cv2.IMWRITE_JPEG_QUALITY,95])
    return True,(x1,y1,x2,y2)

def write_label(path,number,bbox,image_size):
    x1,y1,x2,y2=bbox; xc=((x1+x2)/2)/image_size; yc=((y1+y2)/2)/image_size; w=(x2-x1+1)/image_size; h=(y2-y1+1)/image_size
...
def main():
    ...
    ap.add_argument("--source-class-id",type=int,default=0,help="YOLO class id representing jersey-number objects")
    ap.add_argument("--remove-pad-ratio",type=float,default=.03,help="Extra padding around old-number bbox before inpainting")
    ap.add_argument("--inpaint-radius",type=int,default=3)
    ap.add_argument("--size-mode",choices=["original","fixed"],default="original",help="original=fit inside old number bbox; fixed=use fixed target fraction")
    ...
    if not valid: raise FileNotFoundError(f"No images with matching class {a.source_class_id} labels found in {a.jersey_dir}")
    ...
    for n in nums:
        ...
        while made<counts[n] and attempts<counts[n]*20:
            ...
            if not ok:continue
            bbox_out=res; write_label(out_path.with_suffix('.txt'),n,bbox_out,a.image_size); x1,y1,x2,y2=bbox_out; rows.append([str(out_path.resolve()),n,x1,y1,x2,y2,str(img_path.resolve()),str(label_path.resolve())]); made+=1
