...
Training-oriented modification:
    ...
    - Complex2D keeps the number large (about 38-52% of crop height) and
      places it near the center/torso region with only small jitter.
...
def _warp_number_object(
    ...
    """
    Apply mild geometric variation to the COMPLETE jersey number.

    The number is never split into individual digits here.  This is
    important for 27/42/66/etc.: the pair must undergo exactly the same
    rotation and perspective transformation.
    """
    ph, pw = patch.shape[:2]

    # Only a small scale variation here.  The final size is controlled by
    # the Complex2D placement code so the number stays large enough.
    ...
    # Mild rotation of the COMPLETE number.
    ...
    # Mild perspective variation, applied to the whole number.
    h, w = patch.shape[:2]
    jitter = random.uniform(0.01, 0.035)
...
def _resize_number_for_jersey(
    number_patch: np.ndarray,
    mask: np.ndarray,
    image_size: int,
) -> tuple[np.ndarray, np.ndarray]:
    """
    Make the synthetic number large enough to read as a real jersey number.

    We size primarily by HEIGHT rather than forcing a fixed width.  This is
    important because a single digit is naturally narrower than a double
    digit, while both should have approximately the same printed height.

    Target height:
        38% - 52% of the final image height.

    This is intentionally larger than the old Complex2D setting.
    """
    ph, pw = number_patch.shape[:2]
    if ph <= 0 or pw <= 0:
        return number_patch, mask

    # A jersey number should be clearly visible, but not fill the entire
    # player crop.  Keep this variation fairly narrow for training stability.
    target_h = int(image_size * random.uniform(0.38, 0.52))
    ...
    # Keep a safe margin around the number.
    max_h = image_size - 8
    max_w = image_size - 8

    if nh > max_h or nw > max_w:
        scale = min(max_h / float(nh), max_w / float(nw))
        nw = max(8, int(round(nw * scale)))
        nh = max(8, int(round(nh * scale)))
...
def _center_jersey_position(
    image_size: int,
    patch_w: int,
    patch_h: int,
) -> tuple[int, int]:
    """
    Place the number around the center of the player/jersey crop.

    We do NOT search the complete image randomly anymore.

    For the 100x100 player/jersey crops used by this pipeline:
      - X is centered with only a small horizontal jitter.
      - Y is centered with only a small vertical jitter.

    This makes the synthetic number consistently appear on the torso
    rather than on the background, head, legs, or image corners.
    """
    center_x = image_size / 2.0
    center_y = image_size / 2.0

    # Small natural variation, but never a completely random position.
    jitter_x = random.uniform(-0.06, 0.06) * image_size
    jitter_y = random.uniform(-0.05, 0.06) * image_size

    x = int(round(center_x - patch_w / 2.0 + jitter_x))
    y = int(round(center_y - patch_h / 2.0 + jitter_y))

    # Keep the complete number inside the crop.
    x = max(2, min(x, image_size - patch_w - 2))
    y = max(2, min(y, image_size - patch_h - 2))

    return x, y


def _blend_number_on_jersey(
    background: np.ndarray,
    number_patch: np.ndarray,
    mask: np.ndarray,
    x: int,
    ...
    """
    Blend only the number pixels onto the jersey.

    The Simple2D coloured background is never copied onto the real image.
    A soft edge, mild brightness variation and tiny local blur make the
    number look printed rather than like a rectangular pasted sticker.
    """
    result = background.copy()

    ph, pw = number_patch.shape[:2]
    x2 = min(result.shape[1], x + pw)
    y2 = min(result.shape[0], y + ph)

    if x2 <= x or y2 <= y:
        return result

    patch = number_patch[:y2-y, :x2-x].astype(np.float32)
    m = mask[:y2-y, :x2-x].astype(np.float32) / 255.0

    # Soft but not overly transparent edge.
    m = cv2.GaussianBlur(m, (3, 3), 0)

    # Keep the number readable.  Previous opacity was too weak for the
    # small Complex2D numbers.
    opacity = random.uniform(0.88, 0.98)
    alpha = np.clip(m * opacity, 0.0, 1.0)

    # Small print/lighting variation.
    brightness = random.uniform(0.92, 1.06)
    patch = np.clip(patch * brightness, 0, 255)

    base = result[y:y2, x:x2].astype(np.float32)
...
def generate_complex2d_class(
    ...
    """
    Generate Complex2D images with jersey-aware placement.

    Main changes from the previous Complex2D implementation:

      1. The COMPLETE number (e.g. "29") remains one object.
      2. The number is extracted from Simple2D without its coloured square.
      3. The number is considerably larger:
             ~38-52% of crop height.
      4. The number is placed near the CENTER of the player/jersey crop.
      5. Position variation is small, not random over the entire image.
      6. Rotation/perspective are applied to the whole number.
      7. Only the number pixels are blended onto the real image.

    Assumption:
      The input directory contains player/jersey crops where the jersey is
      approximately around the center of the crop. This matches the crops
      shown in the user's Complex2D examples.
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if not number_images:
        raise FileNotFoundError(
            f"No Simple2D images found for class {number}: {number_dir}"
        )
    ...
    for idx in range(count):
        ...
        # -------------------------------------------------------------
        # Prepare player/jersey crop
        # -------------------------------------------------------------
        bh, bw = bg.shape[:2]

        if bh < image_size or bw < image_size:
            scale = max(image_size / max(bh, 1), image_size / max(bw, 1))
            bg = cv2.resize(
                bg,
                (
        ...
        # Center crop rather than a random crop.  A random crop can move
        # the player's torso away from the center and defeat jersey-aware
        # placement.
        crop_size = min(bh, bw)
        x0 = max(0, (bw - crop_size) // 2)
        y0 = max(0, (bh - crop_size) // 2)
        ...
        # -------------------------------------------------------------
        # Extract ONLY the number from the Simple2D image
        # -------------------------------------------------------------
        mask = _extract_number_mask(number_img)

        number_patch, mask = _tight_crop(
            number_img,
            mask,
            pad=2,
        )
        ...
        # -------------------------------------------------------------
        # Transform the COMPLETE number together
        # -------------------------------------------------------------
        number_patch, mask = _warp_number_object(
            number_patch,
            mask,
            image_size,
        )
        ...
        # -------------------------------------------------------------
        # Make it jersey-number sized
        # -------------------------------------------------------------
        number_patch, mask = _resize_number_for_jersey(
            number_patch,
            mask,
            image_size,
        )
        ...
        # -------------------------------------------------------------
        # CENTER placement with only small natural variation
        # -------------------------------------------------------------
        x, y = _center_jersey_position(
            image_size,
            pw,
            ph,
        )
        ...
        # -------------------------------------------------------------
        # Blend ONLY the number
        # -------------------------------------------------------------
        bg = _blend_number_on_jersey(
            bg,
            number_patch,
            mask,
            x,
        ...
        # -------------------------------------------------------------
        # Mild final camera degradation
        # -------------------------------------------------------------
        if random.random() < 0.25:
            bg = cv2.GaussianBlur(bg, (3, 3), 0)

        if random.random() < 0.20:
            bg = gaussian_noise(bg, (0.8, 4.0))

        cv2.imwrite(
            str(out_dir / f"{number}_{idx:05d}.jpg"),
